...
本项目是一个基于多智能体架构的大模型驱动工具，旨在通过自然语言自动化生成华为 FusionCompute 虚拟化平台的 Ansible Playbooks。项目通过将需求分析、代码编译和质量审查进行物理隔离，结合本地文档向量化检索（RAG），极大降低了基础设施即代码（IaC）的编写门槛与出错率。
...
*   **API 接口 RAG化**：内置 `ingest.py` 将官方 Word 接口文档及 Ansible 模块指南转化为 ChromaDB 向量知识库，利用 Ollama 计算嵌入，实现精准的本地知识注入。
...
1.  **详细描述工作流**：应尽量明确操作和接口的先后顺序。如果操作或者接口不支持幂等或需要异步等待（会产生Task），应明确提出。
	* **❌ 反例：** 关联每个主机和数据存储。
	* **✅ 正例：** 对于每个数据存储存储，先获取数据存储的基本信息，再遍历每一个主机，通过API<创建数据存储>完成主机关联数据存储操作。
2.  **指明信息提取逻辑**：应尽量明确告知从接口的 Response 中需要提取什么信息传递给下一个接口（例如：“获取返回列表中的首个虚拟机的ID”）。
	* **❌ 反例：** 用户指定一个虚拟机名称...
	* **✅ 正例：** 用户指定一个虚拟机名称，查询到ID和操作系统类型后... 
...
**Example 1: 单一资源GET操作（简单任务可以适当降低提示词要求）**
> 编写一个Ansible Playbook，用户指定一个虚拟机名称，查询这个虚拟机的详细信息，如果查询到多个同名虚拟机则都都处理。输出这个虚拟机的ID和每个磁盘所属的数据存储的名称，打印在控制台上。

**Example 2: 单一资源POST操作**
