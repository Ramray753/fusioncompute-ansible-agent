...
# Exmaple 4: 编写一个Ansible Playbook，用户指定两个CSV文件路径（./host_names.csv和./datastore_names.csv），这两个CSV文件没有表头，host_names.csv代表主机名称列表，每一行都是一个主机名称；datastore_names.csv表示数据存储名称列表，每一行都是一个数据存储名称。对于每个主机，遍历每一个数据存储，完成主机关联数据存储操作，必须顺序执行，严禁并行执行。
"$PYTHON_CMD" crew.py "$@"